...
def write_csv(csv_path, content, mul=False, mod="w"):
...
def analysis_thy(cf, batch_dict, result_dict, output_dir, i):
    # have_gt_flag = True
    # save_visual = False
    # images = batch_dict['inputs0']
    images = batch_dict['inputs']
    # gt_mask = batch_dict['mask']
    # mask_label = batch_dict['mask_label']
    label = batch_dict['label']
    ...
    patientid = (' ').join(patientid)#list to str
    # voi_name=batch_dict['voi_name']
    # voi_name = (' ').join(voi_name)  # list to str
    # predict_cls = result_dict['predict_cls']
    # predict_seg = result_dict['predict_seg']
    ...
    # output_csv_path = os.path.join(output_dir, 'test_class.csv')
    # if not os.path.exists(output_csv_path):
    #     # write_csv(output_csv_path, [['gt0', 'gt1', 'prob0', 'prob1']], mod='w')
    #
            #write_csv(output_dir, ['patientid', 'gt0', 'gt1', 'prob0', 'prob1'], mod='w')
    if i == 0:
        path = open(output_dir, 'a', encoding='gbk', newline='')
        # 基于文件对象构建csv写入对象
        csv_write = csv.writer(path)
        # 构建列表头
        csv_write.writerow(['patientid', 'gt0', 'gt1', 'prob0', 'prob1'])
        for i in range(images.size(0)):  # 0, 1
            # sid = infos['seriesuid'][i]
            gt_tmp = np.array(label.cpu())[i]
            ...
            csv_write.writerow([patientid, gt_tmp[0], gt_tmp[1], prob_tmp[0], prob_tmp[1]])
...
def analysis_test_output(cf, batch_dict, result_dict):
    ...
    output_dir = 'D:/pytorch/train_data/ling/data/DCE_MRI/jingjing_linbajie/output/test/test_class.csv'
    ...
    if 'thyroid_cls_common' in os.path.basename(cf.exp_source):
        analysis_thy(cf, batch_dict, result_dict, output_dir)
