...
def normalize_image(image, clip_window=(0.0, 4000.0), output_range=None, ntype='normalize'):
    if ntype == 'normalize':
...
class CreateTransformer(object):
    def __init__(self, cf, random_scale=False, random_crop=False, random_flip=False):
        ...
        if random_flip:
            self.random_flip = [np.random.randint(0, 2), np.random.randint(0, 2), np.random.randint(0, 2)]
        else:
            self.random_flip = [0, 0, 0]

    def image_transform_with_bbox(self, data, mask=None, order=1, pad_value=0, centerd=None, bbox=None):
        """
        :param data:
        :param order:
        :param pad_value:
        ...
        :return:
        """
        shape_scale = np.array(self.crop_size) / data.shape
        self.scale = list(shape_scale * self.scale)
        data = zoom(data, self.scale, order=order)
        if mask is not None:
        ...
        if np.any([data.shape[dim] < ps for dim, ps in enumerate(self.crop_size)]):
            new_shape = [np.max([data.shape[dim], ps]) for dim, ps in enumerate(self.crop_size)]
            data = pad_nd_image(data, new_shape, mode='constant', kwargs={'constant_values': pad_value})
            if mask is not None:
        ...
        for ii in range(len(data.shape)):
            if self.random_crop:
                if data.shape[ii] > self.crop_size[ii]:
                    min_crop = np.random.randint(0, data.shape[ii] - self.crop_size[ii]) // 2
                else:
                    min_crop = 0
            else:
                min_crop = (data.shape[ii] - self.crop_size[ii]) // 2

            max_crop = min_crop + self.crop_size[ii]
            data = np.take(data, indices=range(min_crop, max_crop), axis=ii)
            if mask is not None:
        ...
        for ii, flag in enumerate(self.random_flip):
            if flag:
                ...
                if centerd is not None:
                    centerd[:, ii] = data.shape[ii] - centerd[:, ii] - 1
                if bbox is not None:
                    bbox[:, [ii, ii + 3]] = bbox[:, [ii + 3, ii]]
                    bbox[:, ii] = data.shape[ii] - bbox[:, ii]
                    bbox[:, ii + 3] = data.shape[ii] - bbox[:, ii + 3]

        if mask is not None and centerd is not None and bbox is not None:
            return data, mask, centerd, bbox
        elif mask is not None and centerd is not None:
            return data, mask, centerd
        elif mask is not None and bbox is not None:
            return data, mask, bbox
        elif mask is not None:
            return data, mask
        else:
            return data

class CreateTransformer2(object):
    def __init__(self, cf, random_scale=False, random_crop=False, random_flip=False):
        self.crop_size = cf.input_size2
        if random_scale:
            self.scale = [np.random.uniform(cf.da_kwargs['scale'][0], cf.da_kwargs['scale'][1]),
                          np.random.uniform(cf.da_kwargs['scale'][0], cf.da_kwargs['scale'][1]),
                          np.random.uniform(cf.da_kwargs['scale'][0], cf.da_kwargs['scale'][1])]
