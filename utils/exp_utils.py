...
def import_module(name: object, path: object) -> object:
    """
...
def prep_exp(exp_source, exp_result, use_stored_settings=False, is_train=True):
    ...
    :return:
    """
    from config.configs import configs as cf
    ...
    if is_train:
        ...
        if not os.path.exists(exp_result):
            ...
            subprocess.call('cp {} {}'.format(cf.model_path, exp_result), shell=True)
    ...
    if use_stored_settings:
        ...
        cf = cf_file.configs()
        cf.model_path = os.path.join(exp_result, os.path.basename(cf.model_path))  # todo 暂时不使用拷贝的model，因为难以控制引用的backbone,想到更好的解决办法再改
    ...
    cf.select_model_dir = exp_result  # os.path.join(exp_result, 'select_model')
...
def model_select_save(cf, net, optimizer, monitor_metrics, epoch):
    ...
    if epoch % cf.save_model_per_epochs == 0 or epoch == cf.num_epochs:
        epoch_model_dir = os.path.join(cf.select_model_dir, 'epoch_model')  # dce:epoch_model  para ： /para/epoch_model_2
        if not os.path.exists(epoch_model_dir):
            os.mkdir(epoch_model_dir)
        torch.save(net_state_dict, os.path.join(epoch_model_dir, 'model_%03d.pth' % epoch))
